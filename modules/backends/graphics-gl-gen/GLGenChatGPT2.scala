...
object OpenGLCodeGen {
  ...
  def downloadSpec(url: String, outputPath: String): Unit = {
  ...
  def parseSpec(filePath: String, apiFilter: String): List[GLFunction] = {
  ...
  def main(args: Array[String]): Unit = {
    val specs = List(
      ...
      ("https://www.khronos.org/registry/vulkan/specs/1.3/registry/vk.xml", "vulkan.xml", "vulkan", "Vulkan"),
      ("https://gpuweb.github.io/gpuweb/webgpu.xml", "webgpu.xml", "webgpu", "WebGPU")
    )
    
    specs.foreach { case (url, filePath, apiFilter, apiName) =>
      println(s"Downloading $apiName specs...")
      downloadSpec(url, filePath)
      println(s"Parsing $apiName specs...")
      val functions = parseSpec(filePath, apiFilter)
