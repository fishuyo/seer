...
object FFT {
  var length = Audio().config.bufferSize
  var rect = Array.fill(length)(1f)
  var hann = Array.fill(length)(0f)
  for(i<-(0 until length)) hann(i) = 0.5f*(1f-scala.math.cos(2.0*math.Pi*i/(length-1.0)).toFloat)
...
class PhaseVocoder extends AudioSource {
  ...
  var convert = true
  def setConvert(b:Boolean) = { convert = b }

  def setTimeShift(f:Float) = { timeShift = f}
  def setPitchShift(f:Float) = { pitchShift = f}
  def setGain(f:Float) = { gain = f}
  ...
  def convertMagPhase(win:Array[Float]):Array[Float] = {
    ...
    for(i <-(0 to length/2)){		//Go from 0 to N/2 because fftw places (N/2)+1 complex values in spectral array using r2c fft
      ...
      out(2*i) = scala.math.sqrt(re*re + im*im).toFloat

      // compute phase from real and imaginary components
      phase = scala.math.atan2(im,re).toFloat
  ...
  def unconvertMagPhase(win:Array[Float]):Array[Float] = {
    ...
    for(i <-(0 to length/2)){		
      ...
      if( i < length/2){
        out(2*i) = win(2*i)*scala.math.cos(phase).toFloat
        out(2*i+1) = win(2*i)*scala.math.sin(phase).toFloat
        // if( i == 0) println(s"bin 0 im: ${out(1)}, phasediff: $phase")
      } else {
        out(1) = win(2*i)*scala.math.cos(phase).toFloat
  ...
  def shiftPitch(data:Array[Float]): Array[Float] = {
    ...
    for (i <- (0 to length/2)){ 
      var index = scala.math.max((i*pitchShift).toInt,0)
      if (index <= length/2) { 
        val o = out(2*index)
        out(2*index) = o + data(2*i)
  ...
  override def audioIO( io:AudioIO): Unit = { 
    // in:Array[Float], out:Array[Array[Float]], numOut:Int, numSamples:Int){
    val in = io.in(0)
    val out = io.out
    val numSamples = io.config.bufferSize
