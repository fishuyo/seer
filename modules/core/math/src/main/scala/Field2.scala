...
object VecField2 {
  ...
  def setBoundary(f:VecField2) = {
    ...
    f(0,0) = (f(1,0)+f(0,1)) * -0.5f;
    f(0,f.ny-1) = (f(1,f.ny-1)+f(0,f.ny-2)) * -0.5f;
    f(f.nx-1,0) = (f(f.nx-2,0)+f(f.nx-1,1)) * -0.5f;
    f(f.nx-1,f.ny-1) = (f(f.nx-1,f.ny-2)+f(f.nx-2,f.ny-1 )) * -0.5f;
  }
  ...
  def diffuse(f:VecField2) = {
    ...
    for (y <- ( 1 until f.ny-1 ); x <- ( 1 until f.nx-1 )){
      var fxy = f(x,y)
      tmp(x,y) = fxy + (fxy * -4f + f(x+1,y) + f(x-1,y) + f(x,y+1) + f(x,y-1))*a
    }
  ...
  def linearSolve(f0:VecField2, a:Float, a2:Float, itr:Int) = {
    ...
    cfor(0)(_ < itr, _ + 1){ k =>
      cfor(1)(_ < f0.ny-1, _ + 1){ j => //     j <- 1 until f0.ny-1 ) {
      cfor(1)(_ < f0.nx-1, _ + 1){ i => //i <- 1 until f0.nx-1;
        f(i,j) = (f0(i,j) + (f(i-1,j) + f(i+1,j) + f(i,j-1) + f(i,j+1))*a) * a2;
      }}
      setBoundary(f);
    }
    f0.set( f )
