...
class Loop( var seconds:Float=0f, var sampleRate:Int=Audio().config.sampleRate) extends Gen {
  ...
  var recOut = false
  var iobuffer = new Array[Float](Audio().config.bufferSize)
  ...
  override def audioIO( io:AudioIO ) = {
    ...
    var in = io.in(0)
    if(recOut) in = io.out(1)
    val out = io.out
    val numOut = io.config.outputs
    val count = io.config.bufferSize
    ...
    if(recording){ //fresh loop
      ...
      if(startRecord){
        offset = 0 //(latency * Audio().sampleRate).toInt
        startRecord = false
      }
        ...
        if(reversing){
          ...
          if(stacking){ 
            if(offset < count){
              ...
              b.addFromR( in, count-offset, b.wPos, offset )
              offset = 0
            } else offset -= count            
          }
          ...
          if(stacking){
            if(offset < count){
              // val pos = slPos.next
              b.applyGain( decay, count-offset, b.rPos+1)
              b.addFrom( in, count-offset, b.wPos, offset )
              offset = 0
            } else { offset -= count }
          }     
        }
