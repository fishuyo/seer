...
// );
  ...
  def glDrawBuffers (n:Int, bufs:java.nio.IntBuffer): Unit = {println("Err: Not implemented " + Thread.currentThread.getStackTrace()(1).getMethodName)}
...
// );
  ...
  def glBindVertexArray (array:Int): Unit = gl2.bindVertexArray(array)

  // C function void glDeleteVertexArrays ( GLsizei n, const GLuint *arrays )

  def glDeleteVertexArrays (n:Int, arrays:Array[Int], offset:Int=0): Unit = {
    for(i <- 0 until n){
      VertexArrayUtil.deleteVertexArray(arrays(i))
    }
  }
  ...
  def glDeleteVertexArrays (n:Int, arrays:java.nio.IntBuffer): Unit = {
    for(i <- 0 until n){
      VertexArrayUtil.deleteVertexArray(arrays.get(i))
    }
  ...
  def glGenVertexArrays (n:Int, arrays:Array[Int], offset:Int=0): Unit = {
    for(i <- 0 until n){
      val id = VertexArrayUtil.createVertexArray()
      arrays(i) = id 
    }
  }
  ...
  def glIsVertexArray (array:Int): Boolean = gl2.isVertexArray(array)
